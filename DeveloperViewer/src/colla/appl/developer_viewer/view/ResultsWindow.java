...
public class ResultsWindow extends javax.swing.JInternalFrame {
    ...
    public void setListOfTasks(HashMap<String, HashMap<String, CollATask>> taskList) {
        try {
            jTextPane_taskInfo.getDocument().insertString(0, "", null);
        } catch (BadLocationException ex) {
            Exceptions.printStackTrace(ex);
        }
        DefaultListModel<String> model = (DefaultListModel<String>) this.jList_tasks.getModel();
        model.removeAllElements();
        DefaultComboBoxModel<String> comboModel = (DefaultComboBoxModel<String>) jComboBox_resultGroups.getModel();
    ...
    private void initComponents() {
        ...
        jPanel1 = new colla.kernel.util.ImagePane(BackGround.DENSE_GREEN.getPath());
        ...
        setResizable(true);
        setTitle(org.openide.util.NbBundle.getMessage(ResultsWindow.class, "ResultsWindow.title")); // NOI18N

        jSplitPane1.setDividerLocation(150);
        jSplitPane1.setCursor(new java.awt.Cursor(java.awt.Cursor.DEFAULT_CURSOR));

        jList_tasks.setBorder(javax.swing.BorderFactory.createTitledBorder(null, org.openide.util.NbBundle.getMessage(ResultsWindow.class, "ResultsWindow.jList_tasks.border.title"), javax.swing.border.TitledBorder.LEFT, javax.swing.border.TitledBorder.DEFAULT_POSITION)); // NOI18N
        ...
        jTextPane_taskInfo.addMouseMotionListener(new java.awt.event.MouseMotionAdapter() {
        ...
        jComboBox_resultGroups.setBorder(javax.swing.BorderFactory.createTitledBorder(org.openide.util.NbBundle.getMessage(ResultsWindow.class, "ResultsWindow.jComboBox_resultGroups.border.title"))); // NOI18N
        jComboBox_resultGroups.addActionListener(new java.awt.event.ActionListener() {
        ...
        jPanel1Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addComponent(jComboBox_resultGroups, 0, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
                .addContainerGap())
        );
    ...
    private void jComboBox_resultGroupsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jComboBox_resultGroupsActionPerformed
        DefaultComboBoxModel<String> comboModel = (DefaultComboBoxModel<String>) jComboBox_resultGroups.getModel();
    ...
    private void jTextPane_taskInfoMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTextPane_taskInfoMouseClicked
        ...
        if (doc instanceof DefaultStyledDocument) {
            ...
            if (code != null) {
                ...
                File file;
                try {
                    file = devViewer.getTaskResult(groupName, taskName).getFileFromResult();
                ...
                groupName = archive[0];
                taskName = archive[1];
                CollATicket ticket = this.devViewer.getTaskResult(groupName, taskName).getTicket();
                if (!this.devViewer.downloadResult(ticket, groupName)) {
                    JOptionPane.showMessageDialog(this, "Erro: couldn't download result", "Error", JOptionPane.ERROR_MESSAGE);
                } else {
    ...
    public void updateResults() {
        ...
        String group = (String) comboModel.getSelectedItem();
        if (group != null) {
            HashMap<String, HashMap<String, CollATask>> tasksByGroup = devViewer.getTasks();
    ...
    public void showResult(String groupName, String taskName) {
        ...
        try {
            ...
            doc.insertString(doc.getLength(), result.getTaskName() + " sent by " + result.getOwner() + "\n", msgAttr);
            ...
            if (result.hasTicket()) {
                doc.insertString(doc.getLength(), result.getResult().toString() + " (click here to download the result)\n", downloadFileLink);
...
//        System.err.println("desktop suported: "+Desktop.isDesktopSupported());        
        ...
        if (file != null && Desktop.isDesktopSupported()) {
            ...
            try {
                //System.err.println("opening file: " + file.getAbsolutePath().toString());
                desktop.open(file);
    ...
    private void saveNonFileResult(Object result) {
        if (result instanceof Serializable) {
            ...
            JOptionPane.showMessageDialog(this, "Result doesn't implement Serializable and can't be save.", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private File copyFile(File temp, String directory) {
        ...
        String filename = (String) JOptionPane.showInputDialog(this, "Save file as:", temp.getName().substring(0, temp.getName().lastIndexOf('.')));
        if (filename != null) {
            File file = new File(directory, filename + temp.getName().substring(temp.getName().lastIndexOf('.'), temp.getName().length()));
            try {
                if (!file.exists()) {
                    file.createNewFile();
                } else {
                    int count = 1;
                    while ((file = new File(directory, count + "_" + temp.getName())).exists()) {
                        count++;
                    }
                    file.createNewFile();
                }
                // Creates channel at origin
                FileChannel oriChannel = new FileInputStream(temp.getName()).getChannel();
                // Creates channel at destiny
                FileChannel destChannel = new FileOutputStream(file.getAbsolutePath().toString()).getChannel();
