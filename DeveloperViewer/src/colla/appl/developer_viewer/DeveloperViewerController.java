...
public class DeveloperViewerController extends Observable implements Runnable {
    ...
    @Override
    public void run() {
        microServer = new DevMicroServer(this, user, serverIPaddress, serverPortNumber);
    ...
    public void retrieveStoredResultsFromServer() {
        RetrieveStoredResultsMsg outgoing = new RetrieveStoredResultsMsg(user.getName());
        try {
    ...
    public void receiveTaskResult(String groupName, String taskName, CollATask task) {
        ...
        debug("O resultado da tarefa " + task.getTaskID() + " recebido por " + this.user.getName() + " é: " + task.getResult().toString());
     ...
     */
    public void updateResults(String groupName, String taskName, CollATask result) {
        ...
        String tName = this.generateUniqueTaskName(taskName, result.getTaskID());
    ...
    public CollATask getTaskResult(String groupName, String taskName) {
        return this.taskResults.get(groupName).get(taskName);
    }
    ...
    public void setGroupListResults(Set<String> allGroups) {
        //update group list removing those groups that the user already is a member
    ...
    public void createGroup(String groupName) {
        try {
            AskToCreateGroupMsg add = new AskToCreateGroupMsg(groupName, user.getName());
            Socket socket = new Socket(InetAddress.getByName(serverIPaddress), serverPortNumber);
            ObjectOutputStream output = new ObjectOutputStream(socket.getOutputStream());
    ...
    public void confirmGroupCreation(boolean answer, CollAGroup group) {
        if (answer) {
            try {
                displayInfo("Name " + group.getName() + " is available. Sending request to server...");
                //socket.setSoTimeout(timeout);
                ...
                debug("Group " + group.getName() + " could not be created. Connection timeout.", tout);
                displayInfo("Group " + group.getName() + " could not be created. Connection timeout.");
            } catch (IOException io) {
                debug(group.getName() + " could not be created. Connection lost.", io);
                displayInfo(group.getName() + " could not be created. Connection lost.");
            } catch (ClassNotFoundException cnfe) {
                debug(cnfe);
            }
        } else {
            debug("Name " + group.getName() + " has already been taken by another group. Please, try another name.");
            this.displayInfo("Name " + group.getName() + " has already been taken by another group. Please, try another name.");
        }
    }
    ...
    public void joinGroup(String groupName) {
        try {
            ...
            this.displayInfo("A request to join " + groupName + " was sent to its admin");
            //devGUI.closeJoinGroupDialog();
        } catch (SocketTimeoutException tout) {
            debug("Could not send request to join " + groupName + ". Connection timeout.", tout);
            displayInfo("Could not send request to join " + groupName + ". Connection timeout.");
        } catch (IOException io) {
            debug("Could not send request to join " + groupName + ". Connection can't be established.", io);
            displayInfo("Could not send request to join " + groupName + ". Connection can't be established.");
        } catch (ClassNotFoundException cnfe) {
            debug(cnfe);
        }
    }
    ...
    public void shutdown() {
        ...
        user.setOffline();
        try {
            DeveloperViewerDisconnectMsg outgoing = new DeveloperViewerDisconnectMsg(user);
            Socket socket = new Socket(InetAddress.getByName(serverIPaddress), serverPortNumber);
            socket.setSoTimeout(timeout);
            ObjectOutputStream output = new ObjectOutputStream(socket.getOutputStream());
            output.writeObject(outgoing);
        ...
        if (contact != null) {
            if (contact.isOnline()) {
                if (contact.hasValidIP()) {
                    try {
                        ChatDirectMessageMsg messageToSend = new ChatDirectMessageMsg(this.user.getName(), message);
                        Socket socket = new Socket(InetAddress.getByName(contact.getIp()), contact.getPort());
                        socket.setSoTimeout(timeout);
                        ObjectOutputStream output = new ObjectOutputStream(socket.getOutputStream());
                    ...
                    try {
                        TransmitChatMsg messageToSend = new TransmitChatMsg(this.user.getName(), contactName, message);
                        Socket socket = new Socket(InetAddress.getByName(serverIPaddress), serverPortNumber);
                        socket.setSoTimeout(timeout);
                        ObjectOutputStream output = new ObjectOutputStream(socket.getOutputStream());
                        output.writeObject(messageToSend);
     ...
     */
    public void addGroup(String groupName, CollAGroup group, HashMap<String, CollAUser> usersMap) {
    ...
    public boolean downloadResult(CollATicket ticket, String groupName) {
        try {
            ...
            Socket socket = new Socket(InetAddress.getByName(ticket.getHostIPAddress()), ticket.getHostPort());
            socket.setSoTimeout(timeout);
            ObjectOutputStream output = new ObjectOutputStream(socket.getOutputStream());
            output.writeObject(msg);
            output.flush();
     ...
     */
    public void manageGroups(HashMap<String, List<String>> acceptedMembers, HashMap<String, List<String>> refusedMembers) {
        ...
        for (String groupName : this.getUser().getGroups().keySet()) {
            ...
            if (acceptedMembers.get(groupName) != null && acceptedMembers.get(groupName).size() > 0) {
                isModified = true;
            ...
            if (refusedMembers.get(groupName) != null && refusedMembers.get(groupName).size() > 0) {
                isModified = true;
